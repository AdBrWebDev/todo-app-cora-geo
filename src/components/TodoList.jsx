    ...
    <>
        <ul className="mt-5 w-75 mx-auto">
          {items.map(item => (
            ...
            <button className='button is-danger is-small mr-3' onClick={(e) => dispatch(handleDelete(item.id))}><i className="material-icons">delete</i></button>
            <input type="checkbox" checked={props.checked} onChange={(e) => dispatch(softDelete(item.id))} />
            </div>
            </li>
          ))}
        </ul>
